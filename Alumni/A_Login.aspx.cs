            ...
            else
            {
                ...
                finally
                {
                    if (LoginSuccess)
                    {
                        ...
                        if (Account == "aiping_zhang")
                        {
                            Response.Redirect("A_registrationGroupuseradmin.aspx");//在读/转出证明
                        }
                        else if (Account == "scherrie_wang")
                        {
                            Response.Redirect("A_schoolReportUseradmin.aspx");//成绩单
                        }
                        else if (Account == "ifanchen" || Account == "sara_dong" || Account == "jessica_hu0203" || Account == "karry_bai")
                        {
                            Response.Redirect("A_MoralEducationGroupuserAdmin.aspx");//德育组确认
                        }
                        else if (Account == "mia_liu")
                        {
                            Response.Redirect("A_groupLeaderAdmin.aspx");//注册组查看总数据
                        }
                        else
                        {
                            LiteralResult.Text = "<span style=\"color:Red; font-size:13px;\">您没有登录权限，如有需要请联系资讯室董姝翰-5509</span>";//其他人无权限
                        }
